...
class UserListMenu extends IresMenu {
    ...
    function awp_autocomplete_search()
    {
        ...
        $suggestions = [];
        $prepare = $wpdb->prepare("SELECT ID as id,CONCAT(f_name.meta_value,' ',l_name.meta_value,'(',user_login,')') as label
                                         FROM {$wpdb->prefix}users 
                                         JOIN {$wpdb->prefix}usermeta princ
                                         ON ID=princ.user_id
                                         JOIN {$wpdb->prefix}usermeta f_name
                                         ON princ.user_id=f_name.user_id AND f_name.meta_key='first_name'
                                         JOIN {$wpdb->prefix}usermeta l_name  
                                         ON princ.user_id=l_name.user_id AND l_name.meta_key='last_name'
                                         WHERE princ.meta_key LIKE 'wp_capabilities' AND princ.meta_value NOT LIKE '%administrator%'                
                                         AND user_login LIKE %s 
                                         OR princ.meta_key='first_name' AND princ.meta_value LIKE %s
                                         OR princ.meta_key='last_name' AND princ.meta_value LIKE %s         
                                         GROUP BY ID
                                         ORDER BY f_name.meta_value,l_name.meta_value,user_login
                                         LIMIT 10","%".$_REQUEST['term']."%","%".$_REQUEST['term']."%","%".$_REQUEST['term']."%");
        $suggestions = $wpdb->get_results($prepare);
            ...
            <div class="search">
                <form action="" method="get">
                    <p class="search-box">
                        ...
                        <input type="text" class="search-field" id="search" placeholder="Recherche" name="search" value="<?php if(isset($_GET['search'])) echo $_GET['search']; ?>">
