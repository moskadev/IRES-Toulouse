...
class GroupDetailsMenu extends IresMenu {
    ...
    public function analyzeSentData() : void {
        ...
        if (isset($_POST['submitResponsable'])) {
            ...
            $type_message = "error";
            if ($this->group->addResponsable($newResponsable)) {
                $message = "L'identifiant $newResponsableLogin a été ajouté en tant que responsable du groupe {$this->group->getName()}.";
                $type_message = "updated";
            } else if (!$newResponsable) {
