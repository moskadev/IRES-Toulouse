...
class XLSXWriter {
    ...
    protected function writeStylesXML() : string {
        ...
        $file = new XLSXWriterBufferedWriter($temporary_filename);
    ...
    public function countSheetRows(string $sheet_name = '') {
    ...
    public function markMergedCell(string $sheet_name, $start_cell_row, $start_cell_column, $end_cell_row, $end_cell_column) {
        if (empty($sheet_name) || $this->sheets[$sheet_name]->finalized) {
            return;
        }
    ...
    protected function initializeSheet($sheet_name, $col_widths = [], $auto_filter = false, $freeze_rows = false, $freeze_columns = false) {
        ...
        $this->sheets[$sheet_name] = (object) [
            ...
            'row_count' => 0,
            'file_writer' => new XLSXWriterBufferedWriter($sheet_filename),
            'columns' => [],
            'merge_cells' => [],
    ...
    public function writeSheet(array $data, string $sheet_name = '', array $header_types = []) {
    ...
    public function writeSheetHeader(string $sheet_name, array $header_types, $col_options = null) {
    ...
    protected function writeCell(XLSXWriterBufferedWriter &$file, $row_number, $column_number, $value, $num_format_type, $cell_style_idx) {
        ...
        } else if ($num_format_type == 'n_auto' || 1) { //auto-detect unknown column types
    ...
    public function writeSheetRow(string $sheet_name, array $row, $row_options = null) {
        if (empty($sheet_name)) {
            return;
        }
